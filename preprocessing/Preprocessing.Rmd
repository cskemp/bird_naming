...
# Check how well the Holman and Zoë Anindilyakwa ones line up
...
anindilyakwa - tree-pruner-ee9aeff3-6c7d-4b87-aa87-7d38474286fd - new
montagnais - tree-pruner-65a824c3-96d0-4874-a14e-5c16cef86174 - good
rangi - tree-pruner-a6edba71-4114-4d77-a124-33a41b5e040a - good
saami - tree-pruner-9a0ae6e0-7769-4035-b94d-6990fd7a0130 - good
tlingit - tree-pruner-d5b77082-3468-46aa-9dfc-1ea52bd48562 - new
tobelo - tree-pruner-9c9972ae-a991-47a3-9d35-01692750c6e4 - good
tzeltal - tree-pruner-c45e9238-3a1a-4eaa-a980-b810c0b1954c - good
zapotec - tree-pruner-1546ad01-71c3-4e2b-a201-11f91ccccd83 - good
...
for (language in languages) {
  ...
  ## Yeah, thanks. This is Future Zoë going and fixing it all.
  language_data <- bird_data %>% 
    filter(Language == language)
...
colour_data <- read.csv(here("preprocessing/data/colour_data.csv")) %>% 
  rename(ScientificName2021 = sciName_ebird2021,
         cDiversity = n.loci,
  ...
  group_by(ScientificName2021) %>% 
  mutate(num = n()) %>% 
  ungroup() %>% 
  # If there is more than one listing for a bird, this ditches the measurements taken from the female bird
  filter(num == 1 | (num > 1 & sex != "female")) %>% 
  group_by(ScientificName2021) %>% 
  mutate(num = n()) %>% 
  ungroup() %>% 
  filter(num == 1 | (num > 1 & (!is.na(cBlack))) & (sciname_ebird2019 != "Caracara plancus")) %>% 
  # filter(sex != "female") %>% 
...
colour_data$ScientificName2021[colour_data$sciname_ebird2019 == "Saltator coerulescens"] <- "Saltator grandis"

bird_data <- bird_data %>% 
  left_join(colour_data %>% select(-c(sex,sciname_ebird2019)), 
            by = "ScientificName2021",
