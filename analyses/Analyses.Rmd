...
expect_equal(nrow(manual_check_needed), 23)
...
I suppress some warnings in this section when I'm making the table for LaTex, and throughout the rest of notebook whenever I make a LaTeX table. It's just because I turn some values of a column into text, then later turn the column back into numeric and it gets upset that it can't change the text values into numeric.

```{r}
variables <- c("zDist74",
               "zDistPhy",
               "zDistPer")
variables_all <- c("zDist74",
                   "zDist23",
                   "zDistPhy",
                   "zDistPer",
                   "zDistCol")
...
formulas_mlm <- c(as.formula("SameName ~ zDistCol + (1 | Language)"),
                  formulas_mlm) # add the colour model
formulas_mlm <- c(as.formula(
  "SameName ~ zDistPer + zDistCol + (1 | Language)"),
  formulas_mlm) # add the colour plus perceptual model
...
formulas <- c(as.formula("SameName ~ zDistCol"), formulas) # add the colour model
formulas <- c(as.formula("SameName ~ zDistPer + zDistCol"), 
              formulas) # add the colour model
...
# Find the smallest BIC value
 ...
 latex_table <- table1 %>% 
   ...
   mutate(across(names(colour_pairs), 
                 ~ ifelse(suppressWarnings(as.numeric(.)) <= 2 & Statistic == "BIC", 
   ...
   mutate(across(
    ...
    ~ {
      ...
      min_value <- min(suppressWarnings(as.numeric(.))[Statistic == "BIC" & n == 2], 
                       na.rm = TRUE)
      max_value <- min_value + 2
      # Identify the smallest BIC and anything within 2 of the smallest
      ifelse(suppressWarnings(as.numeric(.)) >= min_value & 
               suppressWarnings(as.numeric(.)) <= max_value & 
               Statistic == "BIC" & 
               n == 2,
             # make them pretty
   ...
   mutate(Model = ifelse(row_number() == 1,
    ...
    # latex table for comparing colour and perceptual BICs
    table_order <- c("null", "Per", "Col", "Per+Col")
    
  table_col <- reduce(LanguageVals, 
                  left_join, 
                  by = c("Model", "Variable", "Statistic")) %>% 
    left_join(AllVals,
              by = c("Model", "Variable", "Statistic")) %>% 
    filter(str_detect(Model, "Col") | Model == "Per" | Model == "null") %>% 
    mutate(across(names(small_bic), 
                 ~ if_else(Statistic == "BIC", 
                           . - small_bic[[cur_column()]], 
                           .)),
           Model = factor(Model, levels = table_order)) %>%
    arrange(Model)  %>% 
    mutate_if(is.numeric, round, 2) %>% 
    filter(!is.na(Variable)) %>% 
    group_by(Model) %>% 
    ...
    kable(format = "latex",
          ...
          linesep = "",
          escape = FALSE,
          caption = "BICs for perceptual and colour measures for logistic regression models predicting name sharing.",
          label = "table:resultsLR") %>% 
    kable_styling(latex_options = c("hold_position")) %>% 
    add_header_above(c("Model" = 1, 
                       "Pred." = 1,
  ...
  writeLines(table_col, here("output/ResultsTableLR_col.tex"))
...
variables_all <- c("zIso74", "zIsoPhy", "zIsoPer", "zIsoCol", "zIso23")
...
formulas_mlm <- c(as.formula("Monotypic ~ zIsoCol + (1 | Language)"), formulas_mlm) # add the colour model
formulas_mlm <- c(as.formula("Monotypic ~ zIsoPer + zIsoCol + (1 | Language)"), formulas_mlm) # add the per and colour model
...
formulas <- c(as.formula("Monotypic ~ zIsoCol"), formulas) # add the colour model
formulas <- c(as.formula("Monotypic ~ zIsoPer + zIsoCol"), formulas) # add the colour & per model
...
latex_table <- table4 %>% 
  ...
  mutate(Model = ifelse(row_number()==1,Model, "")) %>% 
    ...
    table_order <- c("null", "Per", "Col", "Per+Col")
    table4_col <- reduce(LanguageVals_LRmt, 
                         left_join, 
                         by = c("Model", "Variable", "Statistic")) %>% 
      left_join(AllVals_LRmt,
                by = c("Model", "Variable", "Statistic")) %>% 
    ...
    mutate(Model = as.character(Model),
      ...
      kable(format = "latex",
            ...
            linesep = "",
            escape = FALSE,
            caption = "BICs for perceptual and colour measures for logistic regression models predicting singleton categories",
            label = "table:resultsLR") %>% 
      kable_styling(latex_options = c("hold_position")) %>% 
      add_header_above(c("Model" = 1, 
                         "Pred." = 1,
    ...
    writeLines(table4_col, here("output/ResultsTableLRmt_col.tex"))
...
# Find the best model
language_models <- models_LRmt[[language]]
best_model_index <- best_models_LRmt$Model[
  best_models_LRmt$Language == language & best_models_LRmt$Model != "null"]
best_model <- language_models[best_model_index]
...
# I do not know why but the last one won't go into the list. This should make it do so.
 ...
 # Make a table with all predictions
table_lrmt_preds <- reduce(all_predictions, rbind) %>% 
  select(-Model) %>% 
  mutate(Predictor = gsub("zIso","",Predictor)) %>% 
  group_by(Language) %>% 
  mutate(Language = ifelse(row_number()==1,
                           Language,
                           ""))

   latex_preds <- table_lrmt_preds %>%
     rename(" " = Language,
            "  " = Predictor) %>%
     kable(format = "latex",
           align = c("l", "l", "c", "c", "c", "c", "c", "c", "c"),
    ...
    writeLines(latex_preds, here("output/PredictionsLRmt.tex"))
...
latex_vifs_nb <- kable(vifs_table_nb,
                      format = "latex",
                      align = c(rep("c", 7)),
                      booktabs = TRUE,
                      linesep = "",
  ...
  write.csv(vifs_table_nb, file = here("output/VIFs_NB.csv"))
...
for (language in languages_plus) {
  ...
  # Filter the models
  language_models <- models_nb[[language]]
  single_models <- language_models[names(language_models) %in% preds]
  language_predictions <- list()
  predictions_df <- data.frame(Language = character(),
                               Model = character(),
                               Predictor = character(),
                               # column for prediction values after pivoting
                               Value = numeric(),  
                               # column for the predictor levels
                               Level = character(), 
                               stringsAsFactors = FALSE,
                               check.names = FALSE)
...
for (i in seq_along(single_models)) {
  ...
  for (var in pred_vars) {
    ...
    if (var == "Language") {
      next # Skip this iteration if the variable is "Language"
        ...
        # Create an integer sequence between min and max
        values <- seq(var_min, var_max)  
        # Create a newdata dataframe with the correct number of rows
        newdata <- data.frame(matrix(rep(NA, 
                                         length(pred_vars) * length(values)),
                                     nrow = length(values),
                                     ncol = length(pred_vars)))
        colnames(newdata) <- pred_vars
        ...
        # create column names based on the values used for prediction
        prediction_cols <- as.character(values)  
        # Create a temp_df
        temp_df <- data.frame(Language = language,
                              Model = model_name,
                              Predictor = var,
        ...
        confint_lower <-c(language,
                          "CI_lower",
                          var,
                          format_sensibly(unname(exp(ci_lwr))))
        
        confint_upper <-c(language,
                          "CI_upper",
                          var,
                          format_sensibly(unname(exp(ci_upr))))
        
        temp_df <- rbind(temp_df, confint_lower, confint_upper)
  ...
  all_predictions_nb[[language]] <- predictions_df
  
  }

table_nb_preds <- reduce(all_predictions_nb, rbind) 

View(table_nb_preds)

write_csv(table_nb_preds, here("data/Predictions_nb.csv"))
...
# Find the best model
language_models <- models_nb[[language]]
best_model_index <- best_models_nb$Model[
  best_models_nb$Language == language & best_models_nb$Model != "null"]
best_model <- language_models[best_model_index]

  language_predictions <- list()

# Make some predictions for all the models
predictions_df <- data.frame(Language = character(),
                             Model = character(),
                             Predictor = character(),
                             `-2sd` = numeric(),
...
table_nb_preds <- reduce(all_predictions, rbind) %>% 
   filter(!(Language=="Tlingit" & (Model == "74" | (Model=="Per")))
          & !(Language == "Tzeltal" & Model != "74+Phy+Per") 
          & !(Language == "Zapotec" & Model == "Per")) %>% 
  group_by(Language, Model) %>% 
  mutate(Model = ifelse(row_number() == 1, Model, "")) %>% 
  ungroup() %>% 
  group_by(Language) %>% 
  mutate(Language = ifelse(row_number() == 1, Language, "")) %>%
  mutate(Predictor = gsub("zDist","",Predictor)) %>% 
  group_by(Language) %>% 
  mutate(Language = ifelse(row_number() == 1,
                           Language,
  ...
  mutate(Predictor = gsub("zIso", "", Predictor))
   ...
   latex_preds <- table_nb_preds %>%
     ...
     kable(format = "latex",
           ...
           linesep = "",
           escape = FALSE,
           caption = "The effects of predictors on number of birds sharing a category",
           label = "table:predsNB") %>%
     kable_styling(latex_options = c("hold_position", "scale_down")) %>%
     add_header_above(c("Language" = 1, "Model" = 1, "Pred." = 1, "Value of Predictor" = 6))

    writeLines(latex_preds, here("output/PredictionsNB.tex"))
...
latex_table <- table5 %>% 
  ...
  mutate(Model = ifelse(row_number()==1,Model, "")) %>% 
    ...
    writeLines(latex_table, here("output/ResultsTableNB_no0.tex"))
...
# Function to get predictions for truncated model

predict_trunc <- function(model, newdata) {
...
# turn predictions into response scale, applying transformation to account for truncation
  response_preds <- exp(l$fit) / (1 - (1 + exp(l$fit) / disp) ^ (-disp))
  
# get confidence intervals
  ci_l <- l$fit - qnorm(0.975) * l$se.fit
  ci_u <- l$fit + qnorm(0.975) * l$se.fit
  
# Transform them
  ci_lwr <- exp(ci_l) / (1 - (1 + exp(ci_l) / disp) ^ (-disp))
  ci_upr <- exp(ci_u) / (1 - (1 + exp(ci_u) / disp) ^ (-disp))

# Put it all into a data frame
  preds <- data.frame(x = newdata,  
                      prediction = response_preds,
                      ci_lower = ci_lwr,
                      ci_upper = ci_upr)
...
for (i in seq_along(single_models)) {
  ...
  for (var in pred_vars) {
    ...
    if (var == "Language") {
      ...
      predictions <- predict_trunc(model, newdata)
      
      # Create column names based on values used for prediction
      ...
      # Transpose the predictions and create the appropriate column structure
      temp_df[prediction_cols] <- t(data.frame(predictions$prediction))
...
# pred_middle <- c(language, "pred_trunc", var,#                    (predictions$prediction))
